...
export async function GET() {
  ...
  try {
    const response = await requestResponseHealth<OutbreakRecord[]>("/api/outbreaks");
    ...
    return NextResponse.json(
      { message: "The outbreak list could not be loaded right now." },
      { status: 502 }
    );
  }
}
...
export async function POST(request: Request) {
  ...
  const createPayload: CreateOutbreakPayload = {
  ...
  if (!createPayload.name || !createPayload.outbreak_type || !createPayload.start_date) {
    return NextResponse.json(
  ...
  try {
    const response = await requestResponseHealth<unknown>("/api/outbreaks", {
      method: "POST",
      headers: {
    ...
    return NextResponse.json(
      { message: "The outbreak could not be created right now." },
      { status: 502 }
    );
  }
}
