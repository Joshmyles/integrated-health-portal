...
import type { UserPermissionsResponse } from "@/src/features/users/types/users";
...
export async function GET(
  ...
  try {
    const response = await requestResponseHealth<UserPermissionsResponse>(
    ...
    return NextResponse.json(
      { message: "The selected user permissions could not be loaded right now." },
      { status: 502 }
    );
  }
}
