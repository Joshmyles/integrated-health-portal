...
import { useMeaslesPatients } from "@/src/features/cif/hooks/use-measles-patients";
import { CifRequestError } from "@/src/features/cif/lib/cif-client";
import type { MeaslesPatient } from "@/src/features/cif/types/cif";
...
function buildPatientSearchText(patient: MeaslesPatient) {
...
export function CifMeaslesWorkspace() {
  const [draftOutbreakId, setDraftOutbreakId] = useState("");
  const [selectedOutbreakId, setSelectedOutbreakId] = useState("");
  const [searchValue, setSearchValue] = useState("");
  const [statusFilter, setStatusFilter] = useState("all");
  const deferredSearch = useDeferredValue(searchValue.trim().toLowerCase());

  const query = useMeaslesPatients(selectedOutbreakId);
  const patients = query.data?.measles_patients ?? [];
      ...
      <section className={styles.panel}>
        <div className={styles.panelHeader}>
          <div>
            ...
            <p className={styles.panelCopy}>
              Load measles patients by outbreak ID using the same CIF interface pattern as VHF,
              with a dedicated query panel, results panel, and shared table styling.
            </p>
          </div>
        ...
        <div className={styles.toolbar}>
          <div className={styles.fieldGroup}>
            <label className={styles.fieldLabel} htmlFor="measles-outbreak-id">
              Outbreak ID
            </label>
            <input
              className={styles.textInput}
              id="measles-outbreak-id"
              inputMode="numeric"
          ...
          <div className={styles.fieldGroup}>
            <label className={styles.fieldLabel} htmlFor="measles-search">
              Search patients
            </label>
            <input
              className={styles.textInput}
          ...
          <div className={styles.toolbarActions}>
            <div className={styles.fieldGroup}>
              <label className={styles.fieldLabel} htmlFor="measles-status">
                Status
              </label>
              <select
        ...
        {query.isError ? (
          <p className={styles.errorMessage}>
            {query.error instanceof CifRequestError
              ? query.error.message
              : "The measles patient list could not be loaded."}
          </p>
        ) : !selectedOutbreakId ? (
          <p className={styles.statusMessage}>
            Enter an outbreak ID to load the live measles patient list.
          </p>
        ) : (
          <p className={styles.statusMessage}>
            {query.isLoading
              ? `Loading measles patients for outbreak ${selectedOutbreakId}...`
      ...
      <section className={styles.panel}>
        <div className={styles.panelHeader}>
          <div>
            ...
            <p className={styles.panelCopy}>
              Live measles patients for the selected outbreak appear below in the same results view
        ...
        {!selectedOutbreakId ? (
          <div className={styles.emptyState}>
            Select an outbreak ID first to load measles patient records.
          </div>
        ) : query.isLoading ? (
          <div className={styles.emptyState}>Loading measles patients...</div>
          ...
          <>
            ...
            <p className={styles.tableFootnote}>
              This view reflects the live measles patients returned for the outbreak ID you
              selected.
            </p>
          </>
        ) : (
          <div className={styles.emptyState}>
            No measles patients matched the current filters.
          </div>
        )}
      </section>
    </div>
